...
load("filtered_TMT_Summary_Data_FC.RData")
load("Quantile_TMT_Summary_Data3.RData") #ACG
Quantile_TMT_Summary_Data = Quantile_TMT_Summary_Data3
```
...
#addDesign <- model.matrix(~addDiseaseGroup)
colnames(addDesign) <- c("Ctl", "AD", "PD","ADPD")
addFit <- lmFit(Quantile_TMT_Summary_Data[,3:nCols], addDesign)
contrast.matrix <- makeContrasts(AD-Ctl, PD-Ctl, ADPD-Ctl, levels=addDesign)
addFit2 <- contrasts.fit(addFit, contrasts = contrast.matrix)
...
intDesign <- model.matrix(~Ctl + AD + PD + AD:PD)
intDesign <- intDesign[,2:5]
colnames(intDesign) <- c("Ctl", "AD", "PD","ADPD")
intFit <- lmFit(Quantile_TMT_Summary_Data[,3:nCols], intDesign)
...
intFit2 <- contrasts.fit(intFit, contrasts = contrast.matrix)
intFit2 <- eBayes(intFit2)
topGenesInt <- topTable(intFit2, coef=1, adjust="BH")
# Extra contrast matrix for the interaction
#contrast.matrix <- makeContrasts((ADPD+AD+PD)-Ctl, levels=intDesign)
#intFit3 <- contrasts.fit(intFit, contrast.matrix)
#intFit3 <- eBayes(intFit3)
#topGenesInt <- topTable(intFit3, coef=1, adjust="BH")
```
...
#adpdSumIntGenes <- topTable(intFit3, coef=1, number=1000000, sort.by="logFC")
...
adpdSumIntGenes$threshold = as.factor(abs(adpdSumIntGenes$logFC) > 0.25 & adpdSumIntGenes$P.Value < 0.05/nGenes)
...
g1 + ggsave(file='FrontalCortex_Volcano_AdditiveModel_CtrlvsAD.png')
...
g2 + ggsave(file='FrontalCortex_Volcano_AdditiveModel_CtrlvsPD.png')
...
g3 + ggsave(file='FrontalCortex_Volcano_AdditiveModel_CtrlvsADPD.png')
...
g4 + ggsave(file='FrontalCortex_Volcano_InteractionModel_CtrlvsAD.png')
...
g5 + ggsave(file='FrontalCortex_Volcano_InteractionModel_CtrlvsPD.png')
...
g6
#g7 = ggplot(data=adpdSumIntGenes, aes(x=logFC, y=-log10(P.Value), colour=threshold)) +
#  geom_point(alpha=0.4, size=1.75) +
#  theme(legend.position = "none") +
#  xlim(c(-2.5, 2.5)) + ylim(c(0, 15)) +
#  xlab("log2 fold change") + ylab("-log10 p-value") + ggtitle('Interaction model: Ctrl vs. AD + PD + ADPD')
#g7 + ggsave(file='FrontalCortex_Volcano_InteractionModel_CtrlvsSum.png')
...
get_max_conn_genes <- function(data, data_mat, ad_ctl_len, pd_ctl_len, adpd_ctl_len, softPower, min_mod_size){
  ...
  c("Dynamic Tree Cut", "Merged dynamic", "R:AD, B:ADPD"),
  dendroLabels = FALSE, hang = 0.03,
  addGuide = TRUE, guideHang = 0.05)
...
ad_ctl <-  as.data.frame((Quantile_TMT_Summary_Data3[as.integer(rownames(adAddGenes[which(adAddGenes$threshold == TRUE),])),]))
rownames(ad_ctl) <- ad_ctl[,1]
ad_ctl_length = length(rownames(ad_ctl))
ad_ctl[,1] <- NULL

pd_ctl <-  as.data.frame((Quantile_TMT_Summary_Data3[as.integer(rownames(pdAddGenes[which(pdAddGenes$threshold == TRUE),])), ]))
rownames(pd_ctl) <- pd_ctl[,1]
pd_ctl_length = length(rownames(pd_ctl))
pd_ctl[,1] <- NULL

adpd_ctl <-  as.data.frame((Quantile_TMT_Summary_Data3[as.integer(rownames(adpdAddGenes[which(adpdAddGenes$threshold == TRUE),])), ]))
...
main = paste("Scale independence - ACG"));
...
main = paste("Mean connectivity - ACG")) 
...
module_labels = get_max_conn_genes(all_diff_data, all_diff, ad_ctl_length, pd_ctl_length, adpd_ctl_length, 10, 2)
# For each cluster
...
ad_ctl <-  as.data.frame((Quantile_TMT_Summary_Data3[as.integer(rownames(adIntGenes[which(adIntGenes$threshold == TRUE),])),]))
rownames(ad_ctl) <- ad_ctl[,1]
ad_ctl_length = length(rownames(ad_ctl))
ad_ctl[,1] <- NULL

pd_ctl <-  as.data.frame((Quantile_TMT_Summary_Data3[as.integer(rownames(pdIntGenes[which(pdIntGenes$threshold == TRUE),])),]))
rownames(pd_ctl) <- pd_ctl[,1]
pd_ctl_length = length(rownames(pd_ctl))
pd_ctl[,1] <- NULL

adpd_ctl <-  as.data.frame((Quantile_TMT_Summary_Data3[as.integer(rownames(adpdIntGenes[which(adpdIntGenes$threshold == TRUE),])),]))
rownames(adpd_ctl) <- adpd_ctl[,1]
adpd_ctl_length = length(rownames(adpd_ctl))
adpd_ctl[,1] <- NULL
...
module_labels = get_max_conn_genes(all_diff_data, all_diff, ad_ctl_length, pd_ctl_length, adpd_ctl_length, 12, 2)
# For each cluster
for(i in unique(module_labels)){
  if(i!=0){
    all_row_indices = which(colors_merge == i)
...
sizeGrWindow(12, 9)
#pdf(file = "Plots/geneDendro-3.pdf", wi = 9, he = 6)
adpd_colors = c(rep("blue",ad_ctl_length), rep("green", pd_ctl_length), rep("red",adpd_ctl_length))
plotDendroAndColors(geneTree, cbind(dynamicColors, mergedColors, adpd_colors),
c("Dynamic Tree Cut", "Merged dynamic", "R:AD, B:ADPD"),
dendroLabels = FALSE, hang = 0.03,
addGuide = TRUE, guideHang = 0.05)
...
dissTOM = 1-TOMsimilarityFromExpr(all_diff_data, power = 18);
...
sizeGrWindow(9,9)
TOMplot(plotTOM, geneTree, moduleColors, main = "Network heatmap plot, all DE genes")
