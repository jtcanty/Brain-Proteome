...
TMT_Summary_Data <- read_excel("/home/john/Documents/Coding/Brain-Proteome/data/TMT_Summary_Data.xlsx")
...
pairs(PCA_CtlvsPD$x[,1:3], col=CtlvsPD_Colors, pch=16)
legend('bottomright',fill=c('black','blue'), legend = c('Ctl','PD'))

pairs(PCA_CtlvsAD$x[,1:3], col=CtlvsAD_Colors, pch=16)
legend('bottomright',fill=c('black','red'), legend = c('Ctl','AD'))

pairs(PCA_CtlvsADPD$x[,1:3], col=CtlvsPDAD_Colors, pch=16)
legend('bottomright',fill=c('black','green'), legend = c('Ctl','PD_AD'))
```

```{r Clustering using K-means and PAM}
