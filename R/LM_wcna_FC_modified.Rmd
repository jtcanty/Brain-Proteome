...
Quantile_TMT_Summary_Data = Quantile_TMT_Summary_Data2
```
...
get_max_conn_genes <- function(data, data_mat, ad_ctl_len, pd_ctl_len, adpd_ctl_len, softPower, min_mod_size){
  ...
  c("Dynamic Tree Cut", "Merged dynamic", "B:ADPD, R:AD"),
  dendroLabels = FALSE, hang = 0.03,
  addGuide = TRUE, guideHang = 0.05)
...
ad_ctl <-  as.data.frame((Quantile_TMT_Summary_Data2[as.integer(rownames(adAddGenes[which(adAddGenes$threshold == TRUE),])),]))
rownames(ad_ctl) <- ad_ctl[,1]
ad_ctl_length = length(rownames(ad_ctl))
ad_ctl[,1] <- NULL

pd_ctl <-  as.data.frame((Quantile_TMT_Summary_Data2[as.integer(rownames(pdAddGenes[which(pdAddGenes$threshold == TRUE),])), ]))
rownames(pd_ctl) <- pd_ctl[,1]
pd_ctl_length = length(rownames(pd_ctl))
pd_ctl[,1] <- NULL

adpd_ctl <-  as.data.frame((Quantile_TMT_Summary_Data2[as.integer(rownames(adpdAddGenes[which(adpdAddGenes$threshold == TRUE),])), ]))
...
main = paste("Scale independence - FC"));
text(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2],
labels=powers,cex=cex1,col="red");
# this line corresponds to using an R^2 cut-off of h
abline(h=0.70,col="red")
# Mean connectivity as a function of the soft-thresholding power
#dev.copy(png, 'scale_free_additive.png')
plot(sft$fitIndices[,1], sft$fitIndices[,5],
xlab="Soft Threshold (power)",ylab="Mean Connectivity", type="n",
main = paste("Mean connectivity - FC"))
...
ad_ctl <-  as.data.frame((Quantile_TMT_Summary_Data2[as.integer(rownames(adIntGenes[which(adIntGenes$threshold == TRUE),])),]))
rownames(ad_ctl) <- ad_ctl[,1]
ad_ctl_length = length(rownames(ad_ctl))
ad_ctl[,1] <- NULL

pd_ctl <-  as.data.frame((Quantile_TMT_Summary_Data2[as.integer(rownames(pdIntGenes[which(pdIntGenes$threshold == TRUE),])),]))
rownames(pd_ctl) <- pd_ctl[,1]
pd_ctl_length = length(rownames(pd_ctl))
pd_ctl[,1] <- NULL

adpd_ctl <-  as.data.frame((Quantile_TMT_Summary_Data2[as.integer(rownames(adpdSumIntGenes[which(adpdSumIntGenes$threshold == TRUE),])),]))
rownames(adpd_ctl) <- adpd_ctl[,1]
adpd_ctl_length = length(rownames(adpd_ctl))
adpd_ctl[,1] <- NULL
